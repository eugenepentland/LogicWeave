...
pub fn getOperationMode(self: *AP33772S) !OpMode {
    var buff: [1]u8 = undefined;
    try self.readRegister(.OPMODE, &buff);
    const opmode: OpMode = @bitCast(buff[0]);
    return opmode;
}
...
pub fn readMask(self: *AP33772S) !Mask {
    try self.readRegister(.MASK, readBuff[1..]);
    return @bitCast(readBuff[1]);
}
