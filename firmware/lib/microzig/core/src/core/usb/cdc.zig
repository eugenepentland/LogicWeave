    ...
    return struct {
        ...
        fn transfer(ptr: *anyopaque, ep_addr: u8, data: []u8) void {
            ...
            if (ep_addr == self.ep_out) {
                // Original: self.rx.write(data) catch {};
                // Test with byte-by-byte copy:
                var i: usize = 0;
                while (i < data.len) : (i += 1) {
                    if (self.rx.writableLength() > 0) {
                        // Assuming LinearFifo has writeByte or similar.
                        // If not, use self.rx.write(&.{data[i]}) catch {}; but this is less efficient.
                        // For this test, let's be explicit if writeByte isn't standard:
                        const byte_slice = data[i .. i + 1];
                        _ = self.rx.write(byte_slice) catch {}; // Write one byte at a time
                    } else {
                        // FIFO full, data would be lost. Consider logging.
                        std.log.warn("CDC RX FIFO full, byte dropped", .{});
                        break;
                    }
                }
                self.prep_out_transaction();
